...
############ Função para formatar os dados do arquivo CSV --alteramos arquivo para conseguir ser recebido no cassandra-- O CASSANDRA NÃO ACEITA DADO DE NÚMEROS COMO INÍCIO DE ATRIBUTO ############
...
########## Função para conectar ao banco de dados MySQL e inserir os dados do arquivo CSV atualizado ##########
def inserir_dados_mysql(host, user, password, database, dados_formatados, tabela):
    ...
    ######################### Preparar a lista de colunas para a inseri o dataset CSGO####################
    ...
    ############ comittando para salvar o dataset ###############
    conexao.commit()
...
dados_formatados = formatar_dados_csv("C:\\Users\\danie\\OneDrive\\Área de Trabalho\\CS_GO_PRO_DB\\cs_go_pro_db\\Scripts\\caminho\\csgo_players.csv")

########################## Chamadas separadas da função para inserir dados no MySQL para cada tabela, se for só uma dá ruim############
inserir_dados_mysql(
    host="localhost",
    user="root",
    password="12345678",
